...
function renderBundleSvg(bundle){
  ...
  const body=layers.filter(layer=>layer.role!=='silhouette-mask').map(layer=>{const mode=layer.blendMode==='source-over'?'normal':(layer.blendMode||'normal');const maskAttribute=mask&&layer.masked!==false?' mask="url(#bundle-mask)"':'';return`<g${maskAttribute} opacity="${layer.alpha}" style="mix-blend-mode:${mode}">${layerMarkup(layer)}</g>`;}).join('');
  return`<svg xmlns="http://www.w3.org/2000/svg" width="600" height="600" viewBox="0 0 600 600"><defs>${maskDef}</defs>${body}</svg>`;
}
